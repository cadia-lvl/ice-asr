...
get_egs_stage=-10
speed_perturb=false
dir=exp/chain/tdnn_lstm # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=final
...
# Be sure to have a valid egs dir, otherwise set remove_egs=true and common_egs_dir=
remove_egs=false
common_egs_dir=exp_wp2/nnet2_online/nnet_ms_a/egs
...
dir=${dir}$suffix
train_set=training_data$suffix
...
if [ $stage -le 9 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/$train_set \
    data/local/lang_wp2 exp/tri4 exp/tri4_lats$suffix
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    --cmd "$decode_cmd" \
    --feat.online-ivector-dir exp_wp2/nnet2_online/ivectors_${train_set} \
