...
# CUSTOM CHANGE: removing --io-opts "$io-opts" if an array error is thrown from slurm script (e.g. --array 1-4%12) when using max_jobs_run
# make sure that you also have get_egs2_custom.sh !
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then

  extra_opts+=(--left-context $nnet_left_context )
  extra_opts+=(--right-context $nnet_right_context )
  echo "$0: calling get_egs2_custom.sh"
  get_egs2_custom.sh $egs_opts "${extra_opts[@]}" \
